<template>
  <nav class="navbar">
    ...
    <div class="navbar-actions">
      ...
      <button class="btn-ghost btn-toggle" @click="toggleTheme" :title="theme === 'dark' ? 'Light Mode' : 'Dark Mode'">
        {{ theme === 'dark' ? '☀️' : '🌙' }}
      </button>
      <a href="https://github.com/xfg0218/MySQL2PG" target="_blank" class="btn-ghost">
      ...
      <RouterLink :to="{ path: '/', hash: '#quickstart' }" class="btn-primary">{{ t.nav.quickStartBtn }}</RouterLink>
    </div>
  </nav>
</template>

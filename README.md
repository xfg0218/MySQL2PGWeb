...
| FAQ | `/faq` | Standalone page — 9 frequently asked questions in native `<details>`/`<summary>` accordion, dual-column grid layout |
...
|-----------|-------------|
| **NavBar** | Fixed top navigation with backdrop blur. SVG logo, route links, EN/中 toggle, ☀️/🌙 toggle, GitHub link, Quick Start CTA |
...
| **FAQ** | 9 FAQ items in native `<details>`/`<summary>` accordion, dual-column grid layout with numbered badges (standalone on `/faq`) |
